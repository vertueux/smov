...
int RobotData::front_board_i2c_bus = 1;
int RobotData::back_board_i2c_bus = 2;
...
rclcpp::Publisher<smov_msgs::msg::DisplayText>::SharedPtr monitor_pub;
...
void RobotNodeHandle::front_topic_callback(smov_msgs::msg::StatesServos::SharedPtr msg) {
  if (robot->state == "None") {
  ...
  if (msg->state_name == robot->state) {
    for (int i = 0; i < 6; i++) {
      if (msg->value[i] < (2 * robot->front_servos_data[i][5]) - robot->front_servos_data[i][6]) {
        ...
        RCLCPP_ERROR(this->get_logger(), "Angle out of range (minimum): Servo number %d", i);
        RCLCPP_ERROR(this->get_logger(), "===========================================");
      } else if (msg->value[i] > robot->front_servos_data[i][6]) {
        RCLCPP_ERROR(this->get_logger(), "===========================================");
        RCLCPP_ERROR(this->get_logger(), "Angle out of range (maximum): Servo number %d", i);
        RCLCPP_ERROR(this->get_logger(), "===========================================");
      } else {
...
void RobotNodeHandle::back_topic_callback(smov_msgs::msg::StatesServos::SharedPtr msg) {
  if (robot->state == "None") {
    robot->state = msg->state_name.c_str();
    RCLCPP_INFO(this->get_logger(), "===========================================");
    RCLCPP_INFO(this->get_logger(), "Detecting a new state: %s", robot->state.c_str());
...
void RobotNodeHandle::end_state_callback(smov_msgs::msg::EndState::SharedPtr msg) {
  if (msg->state_name == robot->state) {
    RCLCPP_INFO(this->get_logger(), "===========================================");
...
void RobotNodeHandle::declare_parameters() {
  // Getting the parameters from the smov_config node.
  auto parameters_client = std::make_shared<rclcpp::SyncParametersClient>(this, "smov_config");

  while (!parameters_client->wait_for_service(std::chrono::seconds(1))) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(this->get_logger(), "Interrupted while waiting for the SMOV Config service. Exiting.");
      rclcpp::shutdown();
    }
    RCLCPP_INFO(this->get_logger(), "SMOV Config service not available, waiting again...");
  }
  ...
  robot->front_board_i2c_bus = parameters[1].as_int();
  robot->back_board_i2c_bus = parameters[2].as_int();

  for (int i = 0; i < 6; i++) {
    robot->front_servos_data.push_back(parameters[i + 3].as_integer_array()); // "+ 3" since parameters[0], etc.. are storing something else.
    robot->back_servos_data.push_back(parameters[i + 9].as_integer_array());
  }
...
void RobotNodeHandle::set_up_servos() {
  ...
  std::array<i2c_pwm_board_msgs::msg::Servo, 6> empty_front_servos;
  std::array<i2c_pwm_board_msgs::msg::Servo, 6> empty_back_servos;
...
void RobotNodeHandle::set_up_topics() {
  front_states_sub = this->create_subscription<smov_msgs::msg::StatesServos>(
      "front_proportional_servos", 1, std::bind(&RobotNodeHandle::front_topic_callback, this, std::placeholders::_1));

  back_states_sub = this->create_subscription<smov_msgs::msg::StatesServos>(
      "back_proportional_servos", 1, std::bind(&RobotNodeHandle::back_topic_callback, this, std::placeholders::_1));

  end_state_sub = this->create_subscription<smov_msgs::msg::EndState>(
      "end_state", 1, std::bind(&RobotNodeHandle::end_state_callback, this, std::placeholders::_1));

  front_prop_pub = this->create_publisher<i2c_pwm_board_msgs::msg::ServoArray>("servos_proportional_" + std::to_string(robot->front_board_i2c_bus), 50);
  if (!robot->use_single_board)
    back_prop_pub = this->create_publisher<i2c_pwm_board_msgs::msg::ServoArray>("servos_proportional_" + std::to_string(robot->back_board_i2c_bus), 50);

  front_stop_servos_client = this->create_client<std_srvs::srv::Empty>("stop_servos_" + std::to_string(robot->front_board_i2c_bus));
  if (!robot->use_single_board)
    back_stop_servos_client = this->create_client<std_srvs::srv::Empty>("stop_servos_" + std::to_string(robot->back_board_i2c_bus));

  // Setting up the monitor publisher.
  monitor_pub = this->create_publisher<smov_msgs::msg::DisplayText>("data_display", 1);
}
