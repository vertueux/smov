...
void RobotNodeHandle::declare_parameters() {
  ...
  for (int i = 0; i < SERVO_MAX_SIZE; i++) { // 7 is the number of data in a single array (in ~/parameters.yaml).
    RobotStates::front_servos_data.push_back(this->get_parameter(node->servo_name[i]).as_integer_array());
    RobotStates::back_servos_data.push_back(this->get_parameter(node->servo_name[i + SERVO_MAX_SIZE]).as_integer_array());
  }
}
...
void RobotNodeHandle::config_servos() {
  ...
  for (int h = 0; h < SERVO_MAX_SIZE; h++) {
    front_config[h].servo = int(RobotStates::front_servos_data[h][0] + 1);
    back_config[h].servo = int(RobotStates::back_servos_data[h][0] + 1);

    front_config[h].center = int(RobotStates::front_servos_data[h][1]);
    back_config[h].center = int(RobotStates::back_servos_data[h][1]);

    front_config[h].range = int(RobotStates::front_servos_data[h][2]);
    back_config[h].range = int(RobotStates::back_servos_data[h][2]);

    front_config[h].direction = int(RobotStates::front_servos_data[h][3]);
    back_config[h].direction = int(RobotStates::back_servos_data[h][3]);
...
void RobotNodeHandle::call() {
  ...
  for (int i = 0; i < SERVO_MAX_SIZE; i++) { 
    RobotStates::front_servos_data[i] = this->get_parameter(node->servo_name[i]).as_integer_array();
    RobotStates::back_servos_data[i] = this->get_parameter(node->servo_name[i + SERVO_MAX_SIZE]).as_integer_array();
  }

  // Updating the pulse for angle.
  RobotStates::pulse_for_angle = this->get_parameter("pulse_for_angle").as_double();
}
