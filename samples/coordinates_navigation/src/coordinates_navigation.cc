#include "coordinates_navigation.h"

void CoordinatesNavigation::init_reader(int echo) {
...
void CoordinatesNavigation::on_start() {
  init_reader(0);
...
void CoordinatesNavigation::on_loop() {
...
void CoordinatesNavigation::on_quit() {
  tcsetattr(STDIN_FILENO, TCSANOW, &old_chars);
}

// This macro creates the node and the main() input, which spins the node.
DECLARE_STATE_NODE_CLASS("coordinates_navigation", CoordinatesNavigation, 10ms)
