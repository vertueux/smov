...
void TrigonometryState::set_legs_distance_to(float value) {
  ...
  float b = acos((pow((*upper_leg_length), 2.0) + pow(value, 2.0) - pow((*lower_leg_length), 2.0)) / (2 * ((*upper_leg_length)) * (value)));
  float c = acos((pow((*upper_leg_length), 2.0) + pow((*lower_leg_length), 2.0) - pow(value, 2.0)) / (2 * ((*upper_leg_length)) * (*lower_leg_length)));
  float theta = M_PI - c;
