...
drive_mode
    _active_drive;                              // Used when converting Twist geometry to PWM values and which servos are for motion.
int _last_servo = -1;
...
bool set_pwm_frequency(const std::shared_ptr<board_msgs::srv::IntValue::Request> req,
...
bool config_servos(const std::shared_ptr<board_msgs::srv::ServosConfig::Request> req,
...
bool config_drive_mode(const std::shared_ptr<board_msgs::srv::DriveMode::Request> req,
...
int main(int argc, char **argv) {
  ...
  if (_controller_io_device == 1) {
    config_srv = node->create_service<board_msgs::srv::ServosConfig>("front_config_servos",
                                                                     &config_servos);                           // 'config' will setup the necessary properties of continuous servos and is helpful for standard servos.
    abs_sub = node->create_subscription<board_msgs::msg::ServoArray>("front_servos_absolute",
                                                                     500,
                                                                     &servos_absolute);             // The 'absolute' topic will be used for standard servo motion and testing of continuous servos.
    rel_sub = node->create_subscription<board_msgs::msg::ServoArray>("front_servos_proportional",
                                                                     500,
                                                                     &servos_proportional); // The 'proportion' topic will be used for standard servos and continuous rotation aka drive servos.
    freq_srv = node->create_service<board_msgs::srv::IntValue>("front_set_pwm_frequency", &set_pwm_frequency);
    mode_srv = node->create_service<board_msgs::srv::DriveMode>("front_config_drive_mode",
                                                                &config_drive_mode);                     // 'mode' specifies which servos are used for motion and which behavior will be applied when driving.
    stop_srv = node->create_service<std_srvs::srv::Empty>("front_stop_servos",
                                                          &stop_servos);                                           // The 'stop' service can be used at any time.
    drive_sub = node->create_subscription<geometry_msgs::msg::Twist>("front_servos_drive",
                                                                     500,
                                                                     &servos_drive);                     // The 'drive' topic will be used for continuous rotation aka drive servos controlled by Twist messages.
  } else {
    ...
    rel_sub =
        node->create_subscription<board_msgs::msg::ServoArray>("back_servos_proportional", 500, &servos_proportional);
