...
#include "front_board_msgs/msg/servo_config_array.hpp"
#include "front_board_msgs/srv/servos_config.hpp"
#include "front_board_msgs/srv/drive_mode.hpp"
#include "front_board_msgs/msg/position.hpp"
#include "front_board_msgs/msg/position_array.hpp"
#include "front_board_msgs/srv/int_value.hpp"
...
public:
    explicit FrontBoardNode(const std::string & node_name="front_board", const std::string & node_namespace="/")
...
void servos_absolute (const std::shared_ptr<front_board_msgs::msg::ServoArray> msg) {
  for (std::vector<front_board_msgs::msg::Servo>::const_iterator sp = msg->servos.begin(); sp != msg->servos.end(); ++sp) {
    int servo = sp->servo;
    int value = sp->value;
...
void servos_proportional (const std::shared_ptr<front_board_msgs::msg::ServoArray> msg) {
  for (std::vector<front_board_msgs::msg::Servo>::const_iterator sp = msg->servos.begin(); sp != msg->servos.end(); ++sp) {
    int servo = sp->servo;
...
bool set_pwm_frequency (const std::shared_ptr<front_board_msgs::srv::IntValue::Request> req, std::shared_ptr<front_board_msgs::srv::IntValue::Response> res) {
  int freq;
  freq = req->value;
...
bool config_servos (const std::shared_ptr<front_board_msgs::srv::ServosConfig::Request> req, std::shared_ptr<front_board_msgs::srv::ServosConfig::Response> res) {
  long unsigned int i;
...
bool config_drive_mode (const std::shared_ptr<front_board_msgs::srv::DriveMode::Request> req, std::shared_ptr<front_board_msgs::srv::DriveMode::Response> res) {
  res->error = 0;
  long unsigned int i;
...
int main (int argc, char **argv) {	
	...
	auto node = std::make_shared<FrontBoardNode>("front_board2");
	...
	rclcpp::Service<front_board_msgs::srv::IntValue>::SharedPtr freq_srv         = node->create_service<front_board_msgs::srv::IntValue>("set_pwm_frequency", &set_pwm_frequency);
	rclcpp::Service<front_board_msgs::srv::ServosConfig>::SharedPtr config_srv   =	node->create_service<front_board_msgs::srv::ServosConfig>("config_servos", &config_servos);			                // 'config' will setup the necessary properties of continuous servos and is helpful for standard servos.
	rclcpp::Service<front_board_msgs::srv::DriveMode>::SharedPtr mode_srv        =	node->create_service<front_board_msgs::srv::DriveMode>("config_drive_mode", &config_drive_mode);		            // 'mode' specifies which servos are used for motion and which behavior will be applied when driving.
	...
	rclcpp::Subscription<front_board_msgs::msg::ServoArray>::SharedPtr abs_sub   = node->create_subscription<front_board_msgs::msg::ServoArray>("servos_absolute", 500, &servos_absolute);		      // The 'absolute' topic will be used for standard servo motion and testing of continuous servos.
	rclcpp::Subscription<front_board_msgs::msg::ServoArray>::SharedPtr rel_sub   = node->create_subscription<front_board_msgs::msg::ServoArray>("servos_proportional", 500, &servos_proportional);	// The 'proportion' topic will be used for standard servos and continuous rotation aka drive servos.
